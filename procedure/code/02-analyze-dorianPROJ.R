# Analyze Hurricane november Twitter data, by Joseph Holler, 2019
...
#<<<<<<< HEAD
tweetsByHour <- ts_data(tweets, by="hours")
...
tweetsNetwork <- network_graph(tweets, c("quote"))

plot.igraph(tweetsNetwork)
...
tweetsText = tweets %>% select(text) %>% plain_tweets()

# parse out words from tweet text
tweetsWords = tweetsText %>% unnest_tokens(word, text)
...
stop_words = stop_words %>% 
  ...
  add_row(word="tornado warning",lexicon = "Search") %>% 
  add_row(word="debris",lexicon = "Search")

#delete stop words from tweetsWords with an anti_join
...
tweetsWordPairs %>%
  ...
  labs(title = "Word Network of Tweets during Tornado",
       x = "", y = "") +
  theme_void()
...
ggplot() +
  ...
  geom_point(data = tweets, aes(x=lng,y=lat),
             colour = 'purple', alpha = .2) +
  labs(title = "Tweet Locations During Tornado")+
...
#<<<<<<< HEAD
tweetssql <- select(tweets,c("user_id","status_id","text","lat","lng"),starts_with("place"))
tweetsTotalsql <- select(tweetsTotal,c("user_id","status_id","text","lat","lng"),starts_with("place"))
#=======
tweetssql <- select(tweets,c("user_id","status_id","text","lat","lng"),
                    starts_with("place"))
#>>>>>>> upstream/main

#write data to the database
dbWriteTable(con,'tweets',tweetssql, overwrite=TRUE)
dbWriteTable(con,'tweetsTotal',tweetsTotalsql, overwrite=TRUE)
...
# SELECT AddGeometryColumn ('schemaname','tweets','geom',4269,'POINT',2, false);
...
#<<<<<<< HEAD
# Set counties with no points to 0 for the november count #already 0?
